...
class ListenerT5DataSet(Dataset):
    def __init__(self, items):
        self.grouped_data = {}
        self.keys = []
        print("Grouping the dataset by work...")
        
        for item in items:
            #  Analyze spectrogram to get unique id
            spec_path = item["spectrogram"]
            try:
                group_id = spec_path.rsplit('#', 1)[0]
            except IndexError:
                group_id = spec_path
            
            if group_id not in self.grouped_data:
                self.grouped_data[group_id] = []
                self.keys.append(group_id)
            
            self.grouped_data[group_id].append(item)
    
        print(f"Grouping complete: raw samples {len(items)} -> unique segments {len(self.keys)}")
    ...
    def __getitem__(self, idx):
        # Get the Group ID for this index
        group_id = self.keys[idx]
        
        # Get all performance versions under this Group
        candidates = self.grouped_data[group_id]
        
        # 3. Core step: randomly sample one version
        # This lets each epoch see different performers while still covering all measures
        item = random.choice(candidates)
        
        try:
            # --- The logic below remains unchanged ---
            # Load audio features
            ...
            if audio_feature.shape[1] > PATCH_LENGTH:
                audio_feature = audio_feature[:, :PATCH_LENGTH]

            audio_feature = torch.tensor(audio_feature, dtype=torch.float32)
        
            # Read abc from the path in jonsl
            ...
            # If the randomly sampled item is broken, recurse and draw another sample
            # Note: this may draw data from another Group, which is acceptable in rare error cases
            random_idx = random.randint(0, len(self.keys) - 1)
...
if __name__ == "__main__":
    ...
    train_set = DataLoader(train_set, batch_size=batch_size, collate_fn=collate_batch, sampler=train_sampler, shuffle=(train_sampler is None), drop_last=True)
    eval_set = DataLoader(eval_set, batch_size=batch_size, collate_fn=collate_batch, sampler=eval_sampler, shuffle=(eval_sampler is None), drop_last=False)

    lr_scheduler = get_constant_schedule_with_warmup(optimizer, num_warmup_steps= 1000)
    model = model.to(device)
    optimizer = torch.optim.AdamW(model.parameters(), lr=LEARNING_RATE)

    print(f"'The current LOAD_FROM_PRETRAINED is:', {LOAD_FROM_PRETRAINED} \
        \n The current PRETRAINED_PATH is: {PRETRAINED_PATH}")
    if LOAD_FROM_PRETRAINED and os.path.exists(PRETRAINED_PATH):
        ...
        if checkpoint.get('min_eval_loss'):
            print(f"Successfully Loaded Pretrained Checkpoint at Epoch {checkpoint['epoch']} with Loss {checkpoint['min_eval_loss']}")
        else:
            print(f"Successfully Loaded Pretrained Checkpoint at Epoch {checkpoint['epoch']} with Loss {checkpoint['eval_loss']}")
